      ...
      <Block>
        <div className={`${s.hero} ${s.hero__profile} `}>
          ...
          <div className={s.hero__icon__container}>
            {(instagram ? <a target="_blank" rel="noopener noreferrer"  href={instagram}><Instagram/></a> : '')}
            {(facebook ? <a target="_blank" rel="noopener noreferrer" href={facebook}><Facebook/></a> : '')}
            {(twitter ? <a target="_blank" rel="noopener noreferrer" href={twitter}><Twitter/></a> : '')}
          </div>
          {/*現在ログイン中のアカウントのみ表示*/}
