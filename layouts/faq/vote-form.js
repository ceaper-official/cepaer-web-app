  ...
  return (
    ...
    <h5>この回答はお役に立ちましたか？</h5>
      <div className="button-choice-wrapper">
        <Link href="/edit">
          <button  className="button button-snow button-choice-l "onClick={this.selectYes}>はい
        </button></Link>

      <button  className="button  button-snow button-choice-r"onClick={this.selectNo}>いいえ
      </button>
        </div>
     </div>
 ...
 <label class="answer-label"><input type="checkbox"/><span>自分の質問に対する回答が見つからなかった</span></label>
     <label class="answer-label"><input type="checkbox"/><span>その他</span></label>
       ...
       <textarea className="feedback-area" placeholder="他に原因はありますか？"></textarea>
        ...
        <div className="vote-submit"><button className="button button-snow"onClick={this.selectSubmit}>フィードバックを送る
        </button></div>
              </div>
    </div>
  );
